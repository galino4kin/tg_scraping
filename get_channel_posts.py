...
# Numeric peer id only, e.g. -1001234567890
...
def message_to_record(msg: Message) -> Dict[str, Any]:
    """
    Extract as many metadata fields as possible for a single channel post.
    """
    rec: Dict[str, Any] = {
        # basic fields
        "id": msg.id,
...
async def fetch_channel_history() -> None:
    ...
    entity = await client.get_entity(CHANNEL_PEER_ID)
    if not isinstance(entity, Channel):
        print(f"[!] Peer {CHANNEL_PEER_ID} is not a Channel (type={type(entity)})")

    peer = PeerChannel(entity.id)
    peer_id_str = str(CHANNEL_PEER_ID)
    title = getattr(entity, "title", peer_id_str)

    os.makedirs(OUTPUT_DIR, exist_ok=True)
    output_path = os.path.join(OUTPUT_DIR, f"{peer_id_str}_channel_posts.csv")

    print(f"[+] Channel: {title} (peer id={peer_id_str})")
    ...
    try:
        while True:
            ...
            for msg in messages:
                ...
                # init writer + header on first real record
                if writer is None:
                ...
                # prepare row: JSON for complex, escape newlines in strings
                row: Dict[str, Any] = {}
                for k, v in rec.items():
                    if isinstance(v, (dict, list)):
                        row[k] = json.dumps(v, ensure_ascii=False)
                    elif isinstance(v, str):
                        # escape line breaks so CSV stays one physical line per row
            ...
            offset_id = min_id_in_batch  # go further back
